...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        grad = np.multiply(self.output, gradOutput)
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        denominator = (np.var(input, axis=0) + self.EPS) ** 0.5
        mu = np.mean(input, axis=0)
        
        self.gradInput = gradOutput / denominator
        self.gradInput -= np.sum(gradOutput, axis=0) / (batch_size * denominator)
        self.gradInput -= (input - mu) * np.sum(gradOutput * (input - mu), axis=0) / (batch_size * (denominator ** 3))
...
class LeakyReLU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = gradOutput * np.ones_like(input) * (input > 0) + gradOutput * np.ones_like(input) * self.slope * (input <= 0)
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = gradOutput * (input > 0) + gradOutput * (self.alpha + self.output) * (input <= 0)        
        return self.gradInput
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            current_var -= var_first_moment * lr_t /             (var_second_moment ** 0.5 + config['epsilon'])
